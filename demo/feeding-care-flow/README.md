...
3. 居家端教学按最新版 Figma 依次展示 `V4 Setup` 的 Step 1 和 Step 2；两步右上角均提供 Cozy 图标，可进入 AI 助手并返回真实来源页。
4. 院端和居家端控制页均通过 `Switch` 进入独立模式列表，不再先打开模式切换底部弹层；List 左上角 `×` 始终关闭列表并返回对应控制页。Manual 模式和 `Programs` 卡片正文点击后先显示二次确认窗口，确认 `Switch` 才应用并返回各自控制页，取消则保留当前模式。`Details` 进入独立韵律详情页并支持返回 List；也可创建并命名自定义模式，编辑两段吸乳参数、查看模式说明，点击 `Save` 后返回 List。
5. 两端点击 `Start Pumping` 后会自动进入泌乳启动和佩戴检测；顶部统一显示两步的当前、通过和未开始状态。佩戴检测通过页停留 3 秒后显示开始吸乳 toast，并自动进入吸乳中，无需再次点击 `Start pumping`。
6. 四种手动选项均属于 `Standard mode`；切换到 `Expression` 时顶部模式标题保持不变，仅更新图标选中态并在档位旁显示 `Best level test`，测试结果会回填为预设吸乳档位。
