...
3. 居家控制页可通过 `Switch` 进入模式列表，创建并命名自定义模式，编辑两段吸乳参数、查看模式说明，保存后回到控制页继续使用。
4. 两端点击 `Start Pumping` 后，先从底部弹出穿戴提示；确认佩戴完成后再依次完成泌乳启动、佩戴检测和舒适负压三步检查。
   顶部统一显示当前、通过和未开始状态；步骤内部切换仅更新内容，不重复触发弹框入场。
5. 吸乳中页面长按 `Hold to finish` 1.2 秒结束本次吸乳，保存记录后进入数据页。
6. 吸乳中页面默认沿用刚刚确认的舒适负压档位，并可通过 `− / +` 在 1–12 档之间继续调节。
7. 两个 Demo 中所有可见返回按钮均可回到实际访问的上一个页面；没有访问历史时按当前流程顺序回退。
8. 两个入口分别使用独立的 `localStorage` 状态，互不读取、修改或重置对方的进度。
