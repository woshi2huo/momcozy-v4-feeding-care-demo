...
6. 吸乳中页面长按 `Hold to finish` 1.2 秒结束本次吸乳，保存记录后进入数据页。
7. 吸乳中页面沿用控制页预设档位，并可通过 `− / +` 在 1–12 档之间继续调节。
8. 两个 Demo 中所有可见返回按钮均可回到实际访问的上一个页面；没有访问历史时按当前流程顺序回退。
9. 两个入口分别使用独立的 `localStorage` 状态，互不读取、修改或重置对方的进度。
