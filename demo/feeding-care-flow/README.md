...
4. 院端和居家端控制页均通过 `Switch` 进入独立模式列表，不再先打开模式切换底部弹层；Manual 模式和 `Programs` 卡片正文点击后先显示二次确认窗口，确认 `Switch` 才应用并返回各自控制页，取消则保留当前模式。`Details` 进入独立韵律详情页并支持返回 List；也可创建并命名自定义模式，编辑两段吸乳参数、查看模式说明，点击 `Save` 后返回 List。
